...
import {
  ...
  saveMoNuoc,
  uploadHinhMo,
} from "@/app/payments/mo-nuoc-actions";
import { useAuth } from "@/lib/hooks/useAuth";
import {
  getMoNuocCache,
  setMoNuocCache,
  getLastMoNuocDate,
} from "@/lib/mo-nuoc-cache";
...
const isLocked = (t: string) => {
  if (!t) return false;
  const u = t.toUpperCase().normalize("NFC"); // normalize Unicode diacritics
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-4">
          <span
            className={`inline-block px-3 py-1 rounded-full text-sm font-bold ${isLocked(row.TinhTrang) ? "bg-red-100 text-red-700" : isOpened(row.TinhTrang) ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}
          >
          ...
          {infoRows
            ...
            .map((r) => (
              <div key={r.label} className="flex gap-3 text-sm">
                ...
                <span
                  className={`font-semibold text-gray-800 ${r.label === "Tình trạng" ? (isLocked(r.value) ? "text-red-600" : "text-blue-600") : ""}`}
                >
                  {r.value}
                </span>
              </div>
          ...
          <div className="px-6 pb-4 grid grid-cols-2 gap-3">
            {row.HinhKhoa && (
              <div>
                ...
                <a
                  href={proxyUrl(row.HinhKhoa)}
                  target="_blank"
                  rel="noreferrer"
                >
                  <img
                    src={proxyUrl(row.HinhKhoa)}
                    alt="Hình khóa"
...
function MoNuocModal({
  ...
  const handleSave = async () => {
    if (!imageDataUrl) {
      setError("Vui lòng chụp/chọn ảnh mở nước trước khi lưu");
      return;
    }
    if (!ghiChu) {
      setError("Vui lòng chọn ghi chú mở nước");
      return;
    }
    setError(null);
    setIsSaving(true);
    ...
    try {
      ...
      const hinhMoPath =
        uploadRes.success && uploadRes.path ? uploadRes.path : "";
      if (hinhMoPath) {
      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          >
            ...
            <option value="Khách hàng xác nhận có nước">
              Khách hàng xác nhận có nước
            </option>
            <option value="Đã gọi điện báo khách hàng">
              Đã gọi điện báo khách hàng
            </option>
          </select>
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center text-xl shrink-0 ${locked ? "bg-red-100" : "bg-teal-100"}`}
          >
            {locked ? "🔒" : "💧"}
          </div>
          <div>
        ...
        <span
          className={`px-2.5 py-1 rounded-full text-xs font-bold shrink-0 ${locked ? "bg-red-100 text-red-700" : opened ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`}
        >
          {row.TinhTrang || "N/A"}
        </span>
      </div>
...
export default function MoNuocTab() {
  ...
  const [selectedDate, setSelectedDate] = useState(() => {
    if (typeof window !== "undefined") {
      const cachedDate = getLastMoNuocDate();
  ...
  const load = useCallback(
    async (date: string, forceRefresh = false) => {
      ...
      try {
        ...
        if (!forceRefresh) {
          const cached = getMoNuocCache(target);
          if (cached && cached.length > 0) {
            setRows(cached as MoNuocRow[]);
            setLoading(false);
            return;
          }
        }

        const data = await fetchMoNuocByDate(target);
        setRows(data as MoNuocRow[]);
        setMoNuocCache(target, data as MoNuocRow[]);
      } catch (err: any) {
        setError(err.message);
      } finally {
        setLoading(false);
  ...
  const filtered = rows.filter((r) => {
    ...
    return `${r.DanhBa} ${r.TenKH} ${r.SoNha} ${r.Duong}`
      .toLowerCase()
      .includes(term);
  });
  ...
  const handleMoNuocSuccess = (rowId: string, updated: Partial<MoNuocRow>) => {
    setRows((prev) => {
      const newRows = prev.map((r) =>
        r.id === rowId ? { ...r, ...updated } : r,
      );
      setMoNuocCache(selectedDate, newRows);
      return newRows;
    });
    setMoNuocRow(null);
  };
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between">
          ...
          <div className="flex gap-3 flex-wrap items-center">
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs font-bold text-teal-700 whitespace-nowrap">
                📅 Ngày CPMN:
              </label>
              <div className="relative">
                <div className="px-3 py-1.5 border border-teal-200 rounded-lg text-sm font-bold text-gray-700 bg-white flex items-center gap-2 cursor-pointer min-w-[130px] justify-between">
                  <span>
                    {selectedDate
      ...
      {loading ? (
        ...
        <div className="py-20 text-center text-gray-400">
          ...
          <div className="text-base font-medium">
            Không có dữ liệu mở nước hôm nay
          </div>
        </div>
      ) : (

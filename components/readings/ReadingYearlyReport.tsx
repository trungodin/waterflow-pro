...
export default function ReadingYearlyReport() {
    ...
    // 3. SHARED Analysis State for Dot, GB, CoCu (Comparison)
    const [sharedYear1, setSharedYear1] = useState(currentYear)
    const [sharedPeriod1, setSharedPeriod1] = useState(new Date().getMonth() + 1)

    // Default Period 2 to previous month or year
    const [sharedYear2, setSharedYear2] = useState(new Date().getMonth() === 0 ? currentYear - 1 : currentYear)
    const [sharedPeriod2, setSharedPeriod2] = useState(new Date().getMonth() === 0 ? 12 : new Date().getMonth())
    ...
    useEffect(() => {
        const fetch = async () => {
            setLoadingDot(true)
            const res = await getReadingDotComparisonTwo(sharedYear1, sharedPeriod1, sharedYear2, sharedPeriod2)
            setDotData(res)
            setLoadingDot(false)
        }
        fetch()
    }, [sharedYear1, sharedPeriod1, sharedYear2, sharedPeriod2])

    // 4. Fetch GB Data
    useEffect(() => {
        const fetch = async () => {
            setLoadingGB(true)
            const res = await getReadingGBComparisonTwo(sharedYear1, sharedPeriod1, sharedYear2, sharedPeriod2)
            setGbData(res)
            setLoadingGB(false)
        }
        fetch()
    ...
    useEffect(() => {
        const fetch = async () => {
            setLoadingCoCu(true)
            const res = await getReadingCoCuComparisonTwo(sharedYear1, sharedPeriod1, sharedYear2, sharedPeriod2)
            setCocuData(res)
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="overflow-y-auto max-h-[350px]">
                            <table className="min-w-full divide-y divide-gray-200 text-sm">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {[...yearlyData].sort((a, b) => Number(b.Nam) - Number(a.Nam)).map((row) => (
            ...
            {/* SECTIONS 2 & 3: COMPARISON & DOT ANALYSIS */}
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                    <div className="flex flex-col gap-4 mb-4">
                        <div className="flex flex-col xl:flex-row justify-between xl:items-center gap-4">
                            ...
                            <div className="flex items-center gap-2 p-1.5 bg-blue-50 rounded-lg border border-blue-100 w-fit self-end xl:self-auto">
                                <div className="flex items-center gap-1">
                                    <span className="w-2 h-2 rounded-full bg-pink-600"></span>
                                    <select value={sharedYear2} onChange={(e) => setSharedYear2(Number(e.target.value))} className="px-2 py-1 border border-gray-300 rounded text-xs font-bold text-gray-700 bg-white focus:outline-none focus:ring-1 focus:ring-pink-500 cursor-pointer">
                                        {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
                                    </select>
                                    <select value={sharedPeriod2} onChange={(e) => setSharedPeriod2(Number(e.target.value))} className="px-2 py-1 border border-gray-300 rounded text-xs font-bold text-pink-700 bg-pink-50 focus:outline-none focus:ring-1 focus:ring-pink-500 cursor-pointer">
                                        {periods.map(p => <option key={p} value={p}>Kỳ {p}</option>)}
                                    </select>
                                </div>
                                <span className="text-gray-400 font-extrabold text-xs">VS</span>
                                <div className="flex items-center gap-1">
                                    <select value={sharedYear1} onChange={(e) => setSharedYear1(Number(e.target.value))} className="px-2 py-1 border border-gray-300 rounded text-xs font-bold text-gray-700 bg-white focus:outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer">
                                        {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
                                    </select>
                                    <select value={sharedPeriod1} onChange={(e) => setSharedPeriod1(Number(e.target.value))} className="px-2 py-1 border border-gray-300 rounded text-xs font-bold text-indigo-700 bg-indigo-50 focus:outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer">
                                        {periods.map(p => <option key={p} value={p}>Kỳ {p}</option>)}
                                    </select>
                        ...
                        {loadingDot ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={dotData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                                        ...
                                                        <div className="space-y-1">
                                                            <div className="flex justify-between items-center gap-2">
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-xs text-indigo-700 font-semibold">Kỳ {sharedPeriod1}/{sharedYear1}:</span>
                                                                </div>
                                                                <span className="font-bold text-gray-800">{formatNumber(v1)}</span>
                                                            </div>
                                                            <div className="flex justify-between items-center gap-2">
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-xs text-pink-700 font-semibold">Kỳ {sharedPeriod2}/{sharedYear2}:</span>
                                                                </div>
                                                                <span className="font-bold text-gray-800">{formatNumber(v2)}</span>
                                                            </div>
                                                            <div className={`flex justify-between items-center gap-2 pt-2 mt-1 border-t border-gray-100 font-bold ${color}`}>
                                    ...
                                    />
                                    <Bar dataKey="Val1" name={`Kỳ ${sharedPeriod1}/${sharedYear1}`} fill="#4f46e5" radius={[3, 3, 0, 0]}>
                                        <LabelList
                                            dataKey="Val1"
                                            content={(props: any) => {
                                    ...
                                    <Bar dataKey="Val2" name={`Kỳ ${sharedPeriod2}/${sharedYear2}`} fill="#db2777" radius={[3, 3, 0, 0]}>
                                        <LabelList
                                            dataKey="Val2"
                                            content={(props: any) => {
                                                const { x, y, width, index } = props
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-2">
                        ...
                        <div className="flex items-center gap-3 text-xs font-bold">
                            <div className="flex items-center gap-1.5 p-1 px-2 rounded-md bg-teal-50 border border-teal-100 text-teal-700">
                                <span className="w-2 h-2 rounded-full bg-teal-500"></span>
                                <span>Kỳ {sharedPeriod1}/{sharedYear1}</span>
                            </div>
                            <div className="flex items-center gap-1.5 p-1 px-2 rounded-md bg-orange-50 border border-orange-100 text-orange-700">
                                <span className="w-2 h-2 rounded-full bg-orange-500"></span>
                                <span>Kỳ {sharedPeriod2}/{sharedYear2}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        {loadingGB ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={gbData} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                                                        ...
                                                        <div className="space-y-1">
                                                            <div className="flex justify-between items-center gap-2">
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-xs text-teal-700 font-semibold">Kỳ {sharedPeriod1}/{sharedYear1}:</span>
                                                                </div>
                                                                <span className="font-bold text-gray-800">{formatNumber(v1)}</span>
                                                            </div>
                                                            <div className="flex justify-between items-center gap-2">
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-xs text-orange-700 font-semibold">Kỳ {sharedPeriod2}/{sharedYear2}:</span>
                                                                </div>
                                                                <span className="font-bold text-gray-800">{formatNumber(v2)}</span>
                                                            </div>
                                                            <div className={`flex justify-between items-center gap-2 pt-2 mt-1 border-t border-gray-100 font-bold ${color}`}>
                                    ...
                                    />
                                    <Bar dataKey="Val1" name={`Kỳ ${sharedPeriod1}/${sharedYear1}`} fill="#14b8a6" radius={[4, 4, 0, 0]}>
                                    ...
                                    <Bar dataKey="Val2" name={`Kỳ ${sharedPeriod2}/${sharedYear2}`} fill="#f97316" radius={[4, 4, 0, 0]}>
                ...
                {/* SECTION 5: COCU ANALYSIS */}
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-2">
                        <h3 className="font-bold text-gray-900 text-lg flex items-center gap-2">📏 Sản Lượng Theo Cỡ Đồng Hồ</h3>
                        <div className="flex items-center gap-3 text-xs font-bold">
                            <div className="flex items-center gap-1.5 p-1 px-2 rounded-md bg-cyan-50 border border-cyan-100 text-cyan-700">
                                <span className="w-2 h-2 rounded-full bg-cyan-500"></span>
                                <span>Kỳ {sharedPeriod1}/{sharedYear1}</span>
                            </div>
                            <div className="flex items-center gap-1.5 p-1 px-2 rounded-md bg-amber-50 border border-amber-100 text-amber-700">
                        ...
                        {loadingCoCu ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={cocuData} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                                                        ...
                                                        <div className="space-y-1">
                                                            <div className="flex justify-between items-center gap-2">
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-xs text-cyan-700 font-semibold">Kỳ {sharedPeriod1}/{sharedYear1}:</span>
                                                                </div>
                                                                <span className="font-bold text-gray-800">{formatNumber(v1)}</span>
                                                            </div>
                                                            <div className="flex justify-between items-center gap-2">
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-xs text-amber-700 font-semibold">Kỳ {sharedPeriod2}/{sharedYear2}:</span>
                                                                </div>
                                                                <span className="font-bold text-gray-800">{formatNumber(v2)}</span>
                                                            </div>
                                                            <div className={`flex justify-between items-center gap-2 pt-2 mt-1 border-t border-gray-100 font-bold ${color}`}>
                                    ...
                                    />
                                    <Bar dataKey="Val1" name={`Kỳ ${sharedPeriod1}/${sharedYear1}`} fill="#06b6d4" radius={[4, 4, 0, 0]}>
                                    ...
                                    <Bar dataKey="Val2" name={`Kỳ ${sharedPeriod2}/${sharedYear2}`} fill="#f59e0b" radius={[4, 4, 0, 0]}>

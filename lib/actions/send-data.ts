 ...
 */
export async function sendToListSheet(
  selectedData: any[],
  assignGroup: string,
  assignDate: string,
) {
  try {
    if (!selectedData || selectedData.length === 0)
      return { success: false, message: "Không có dữ liệu để gửi" };
    ...
    const upsertData = selectedData.map((item, index) => {
      ...
      const hopBaoVe =
        item.HopBaoVe === true || String(item.HopBaoVe).toLowerCase() === "true"
          ? "1"
          : "0";
      const tongKy = item.TongKy ? parseInt(item.TongKy.toString()) : null;
      const tongTien = item.TongNo
        ? parseFloat(item.TongNo.toString().replace(/,/g, ""))
        : null;
    ...
    const { error } = await supabase
      ...
      .upsert(upsertData, { onConflict: "ref_id" });
    ...
    return {
      success: true,
      message: `Gửi thành công ${upsertData.length} khách hàng cho nhóm ${assignGroup}.`,
    };
  } catch (error: any) {
 ...
 */
export async function sendToNhacNoSheet(
  selectedData: any[],
  assignDate: string,
) {
  try {
    ...
    return {
      success: true,
      message: `Gửi nhắc nợ thành công ${upsertData.length} khách hàng.`,
    };
  } catch (error: any) {
